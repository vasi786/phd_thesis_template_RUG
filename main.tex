...
\PassOptionsToPackage{automark, headsepline, plainheadsepline}{scrlayer-scrpage}
...
\hypersetup{%
    pdfauthor={AUTHOR},
    pdftitle={MAIN TITLE},
    pdfsubject={SUBJECT},   % Or whatever the subject of your thesis is
...
\linespread{1.05}  % default is 1.0
\setlength{\parindent}{1.5em}

%%%%%%%%%%%%%
% below two commands will make the toc chapter names lower case and bold
...
\begin{document}
    ...
    \pagestyle{scrheadings}  % not just headings; this gives you your header/footer control
    \KOMAoptions{open=right}                      % Chapters start on the right-hand side in the booklet.
    ...
    \backmatter
    \KOMAoptions{open=right}                      % many of these things fit on two pages, so start on the left pages starting from now on. Can also set to 'any' if you prefer.
    \hypersetup{
        hidelinks
    }                                            % In the list of publications, the links should not be coloured anymore
    %%%%%%%%%%%%%%%%%%%%%%%
    %% setting counter for chapter to 0, to stop the chapterbar from appearing.
